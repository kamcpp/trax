...
func postSetDatabaseName(c *gin.Context) {
	...
	// Re-enable LISTEN/NOTIFY on the new store for event-driven saga processing
	// This is critical because the original Listen() call in traxcoord.go was made on the old store
	listenChannel := "trax_saga_events"
	if err := newStore.Listen(ctx, listenChannel); err != nil {
		common.L.Warn(fmt.Sprintf("Failed to enable LISTEN/NOTIFY on channel '%s' for new store: %v. Falling back to polling.", listenChannel, err), common.F(ctx)...)
	} else {
		common.L.Info(fmt.Sprintf("LISTEN/NOTIFY enabled on channel '%s' for new store", listenChannel), common.F(ctx)...)
	}

	templateChannel := "trax_template_events"
	if err := newStore.Listen(ctx, templateChannel); err != nil {
		common.L.Warn(fmt.Sprintf("Failed to enable LISTEN/NOTIFY on channel '%s' for new store: %v.", templateChannel, err), common.F(ctx)...)
	} else {
		common.L.Info(fmt.Sprintf("LISTEN/NOTIFY enabled on channel '%s' for new store", templateChannel), common.F(ctx)...)
	}

	// Save reference to old store
	oldStore := traxCoordinatorService.GetStore()

	// Replace the store in the coordinator
	// This ensures new requests use the new connection immediately
	traxCoordinatorService.SetStore(newStore)

	common.L.Info("Recreated traxStore with new database connection", common.F(ctx)...)

	// Restart the coordinator to pick up the new database
	// This stops old processing loops and starts new ones with the new store
	common.L.Info("Restarting coordinator processing loops with new database...", common.F(ctx)...)
	if err := traxCoordinatorService.Restart(ctx); err != nil {
		errMsg := fmt.Sprintf("failed to restart coordinator: %v", err)
		common.L.Error(errMsg, common.F(ctx)...)
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": fmt.Sprintf("failed to restart coordinator: %v", err),
		})
		return
	}
	common.L.Info("Coordinator restarted successfully with new database", common.F(ctx)...)

	// Wait for new processing loops to fully establish connections
	// This ensures all goroutines pick up the new database before proceeding
	common.L.Info("Waiting for processing loops to stabilize with new database...", common.F(ctx)...)
	time.Sleep(5 * time.Second)
	common.L.Info("Processing loops stabilized", common.F(ctx)...)

	// Close old store connections synchronously to prevent connection accumulation
	// The 5-second stabilization wait above should have allowed in-flight requests to complete
	if oldStore != nil {
		common.L.Info("Closing old database connections...", common.F(ctx)...)
