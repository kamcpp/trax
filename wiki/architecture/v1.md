...
TRAX runs durable, asynchronous, multi-step workflows where each step can be retried, inspected,
and optionally compensated.
...
Dependent systems own their business workflows, payload schemas, and executor implementations.
TRAX owns the business-neutral workflow mechanics: state, routing, retries, idempotency,
compensation, hierarchy, inspection, and operations.
...
`pkg/mq` is the TRAX RabbitMQ wrapper. Its initialization path should stay limited to TRAX
exchange, queue, and routing setup.
...
The repo is usable as the canonical TRAX home, but some cleanup remains:
...
- `go.mod` and `pkg/common` should be kept narrow enough for TRAX's runtime and tests.
- Generated Swagger packages are expected by daemon API code but may need generation before image builds.
- The local toolchain must be modern enough for the dependencies used by TRAX.
